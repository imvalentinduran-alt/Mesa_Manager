...
const COL_HEADS = ['Cédula', 'Apellido, Nombre', 'Especialidad', '']
...
export default function ProfesoresTab({ session }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setItems(await profesoresService.getAll())
    } catch (e) {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (modal?.id) {
        await profesoresService.update(modal.id, formData)
        showToast('ok', 'Profesor actualizado.')
      } else {
        await profesoresService.create(formData)
        showToast('ok', 'Profesor registrado.')
      }
      setModal(null); load()
    } catch (e) {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      await profesoresService.remove(delTarget.id)
      showToast('ok', 'Profesor eliminado.')
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
          ...
          <tbody>
            ...
            {!loading && !loadErr && items.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-14 text-center">
                <p className="font-body text-sm text-slate-400">No hay profesores registrados aún.</p>
              </td></tr>
            )}
            ...
            {!loading && !loadErr && items.map(item => (
              <tr key={item.id}
                ...
                <td className="px-4 py-3.5 font-mono text-xs text-slate-500">{item.cedula}</td>
                <td className="px-4 py-3.5 font-body font-medium text-slate-800">{item.nombre_completo}</td>
                <td className="px-4 py-3.5 font-body text-slate-500">{item.especialidad}</td>
      ...
      <Modal open={modal !== null} onClose={() => !saving && setModal(null)}
        title={modal?.id ? 'Editar profesor' : 'Nuevo profesor'} width="max-w-md">
        <ProfesorForm initial={modal} onSave={handleSave}
